...
class panel extends views {
    public function start(){
        ...
        if ($permission != true){
            ...
            Core\pager::go_page('login');
            exit;
        }
        $this->PanelActions();
        if (isset($_GET['logout'])){
            if ($login_module->logout()){
                \Core\pager::go_page("403");
            }
        }
        parent::Show();
        parent::setData(array_merge($this->getUserInfos(),$this->Permissions(),$datas));
    }
    private function set_datas_each_parts(){
        ...
        switch ($_GET['parts']){
            ...
            case 'withdrawals_info':
                $wallet = new wallet();
                $info = $wallet->select_withdrawal_requests("WHERE id = '".$_GET['id']."'");
                $datas = [
    ...
    private function WithdrawalRequestActions_Admin(){
        $preg = new preg();
        $wallet = new wallet();
        if ($_GET['parts'] == "withdrawals_info"){
            try{
                if (!isset($_GET['id'])){
                    throw new \Exception("این برداشت یافت نشد");
                }
                $id = $_GET['id'];
                if ($preg->number($id) != 1){
                    throw new \Exception("این برداشت یافت نشد");
                ...
                message::msg_box_session_prepare($e->getMessage(),"danger");
                pager::go_page("/profile/withdrawals");
                exit();
            }
        }
    }
